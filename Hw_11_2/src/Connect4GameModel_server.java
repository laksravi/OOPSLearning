...
public class Connect4GameModel_server implements Observer {
	...
	public Connect4GameModel_server() {
	...
	public Connect4GameModel_server(Connect4GameModel_server aConnect4Field) {
	...
	public void update(Observable arg0, Object arg1) {
		...
		if (arg1 instanceof Integer) {
			checkAndDrop((GameControl_server) arg0);
		}
	}

	/**
	 * Check and drop it in the specified location of gameControl
	 * @param gameControl_server
	 */
	public void checkAndDrop(GameControl_server gameControl_server) {
		int move = gameControl_server.getCurrentgameMove();
		char gameChar = gameControl_server.getCurrentGamePiece();
		if (checkIfPiecedCanBeDroppedIn(move)) {
			dropPieces(move, gameChar);
			if (isItaDraw()) {
				gameControl_server.setGameTie();
			}
			if (didLastMoveWin()) {
				gameControl_server.setGameWin();

			}
		} else {
			gameControl_server.setGameError("Invalid Drop");
		}
