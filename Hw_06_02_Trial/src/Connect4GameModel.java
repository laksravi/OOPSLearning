...
/**
 * This class contains the logic required to implement the Board of the connect4
 * game
 * 
 * @author Lakshmi Ravi
 * @author Aarti Gorde
...
public class Connect4GameModel implements Observer {
	...
	public Connect4GameModel(Connect4GameModel aConnect4Field) {
	 ...
	 * @param gameControl
	 */
	public void checkAndDrop(GameControl gameControl) {
		int move = gameControl.getCurrentgameMove();
		char gameChar = gameControl.getCurrentGamePiece();
		if (checkIfPiecedCanBeDroppedIn(move)) {
			dropPieces(move, gameChar);
			if (isItaDraw()) {
				gameControl.setGameTie();
			}
			if (didLastMoveWin()) {
				gameControl.setGameWin();

			}
		} else {
			gameControl.setGameError("Invalid Drop");
		}
